  ...
  `(let [[v# c#] (~'alts! [~sc ~fc])]
      (try
        (if (= c# ~sc)
          ...
          (throw (js/Error. v#)))
        (finally
          (clojures.core.async/close! ~sc)
          (clojures.core.async/close! ~fc)))))
  ...
  (if (list? forms)                                                                  ; 1
    (if (= (last forms) '...)                                                        ; 2  
      (let [sc (gensym) fc (gensym)] ; sc -> success, fc -> fail                     ; 3
        `(let [~sc (clojures.core.async/chan) ~fc (clojures.core.async/chan)]                ; 4
           (do 
...
```clojure
        `(let [~sc (clojures.core.async/chan) ~fc (clojures.core.async/chan)] 
           (do 
```
...
(transform '(def f1 (.readFile fs "file1" "utf8" ...)))  ; =>
  ; (def f1 
  ;   (clojure.core/let [G__1189 (clojures.core.async/chan) G__1190 (clojures.core.async/chan)] 
  ;     (do 
  ...
  ;             (throw (js/Error. v__1178__auto__))) 
  ;           (finally 
  ;             (clojures.core.async/close! G__1189) 
  ;             (clojures.core.async/close! G__1190)))))))
...
code for the asynchronize [macro here](https://github.com/gilbertw1/clojures-asynchronize/blob/master/src/clojures_asynchronize/macros.clojure).
